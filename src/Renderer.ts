...
export class Renderer {
  ...
  public drawBloom(canvas: p5 | p5.Graphics, stemHeight: number, volume: number, pitch: number, pitchChange: number, leaves: Leaf[], bloomProgress: number, time: number = this.p.millis()): void {
    ...
    for (let layer = 0; layer < layers; layer++) {
      ...
      for (let i = 0; i < petalCount; i++) {
        ...
        canvas.fill(this.p.red(petalColor), this.p.green(petalColor), this.p.blue(petalColor), 25 * layerAlpha * easedProgress);
        canvas.noStroke();
        canvas.beginShape();
  ...
  private drawBloomWithCollapseY(stemHeight: number, volume: number, pitch: number, pitchChange: number, leaves: Leaf[], bloomProgress: number, time: number, collapseY: number): void {
    ...
    for (let layer = 0; layer < layers; layer++) {
      ...
      for (let i = 0; i < petalCount; i++) {
        ...
        this.p.fill(this.p.red(petalColor), this.p.green(petalColor), this.p.blue(petalColor), 35 * layerAlpha * easedProgress);
        this.p.noStroke();
        this.p.beginShape();
    ...
    for (let layer = 0; layer < centerLayers; layer++) {
      ...
      this.p.fill(this.p.red(centerColor), this.p.green(centerColor), this.p.blue(centerColor), alpha);
      this.p.noStroke();
   ...
   * @param bloomProgress 開花の進行度（0-1）
   */
  private drawAmbientGlow(stemHeight: number, volume: number, pitch: number, pitchChange: number, bloomProgress: number): void {
    const seedPos = this.getSeedPosition();
    ...
    for (let i = 0; i < layers; i++) {
      ...
      const alpha = glowIntensity * (1 - layerT * layerT * layerT) * 5; // 8→5に減らす
  ...
  private drawScatteringBloom(stemHeight: number, volume: number, pitch: number, pitchChange: number, leaves: Leaf[], scatterAge: number, time: number, particles: Particle[]): void {
    ...
    for (let layer = 0; layer < layers; layer++) {
      ...
      for (let i = 0; i < petalCount; i++) {
        ...
        this.p.fill(this.p.red(petalColor), this.p.green(petalColor), this.p.blue(petalColor), finalAlpha);
        this.p.noStroke();
        this.p.beginShape();
        this.p.vertex(flowerCenterX, flowerCenterY + depthOffset);
        this.p.vertex(point.x, point.y);
    ...
    for (let layer = 0; layer < layers; layer++) {
      ...
      for (let angle = 0; angle < 360; angle += angleStep) {
        ...
        particles.push({
          ...
          vx: 0,
          vy: 0,
          color: `rgb(${Math.floor(this.p.red(petalColor))}, ${Math.floor(this.p.green(petalColor))}, ${Math.floor(this.p.blue(petalColor))})`,
          alpha: 0.9,
